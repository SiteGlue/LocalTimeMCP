...
import { z } from 'zod';
import { 
  GetBusinessTimeSchema, 
  CheckBusinessHoursSchema, 
  GetTimezoneInfoSchema,
  BusinessTimeResult,
  BusinessHoursResult,
  TimezoneInfoResult,
  ValidationError,
...
export function registerTools(server: McpServer): void {
    ...
    async ({ zipCode, format = "12" }: { zipCode: string; format?: "12" | "24" }) => {
      try {
        ...
        const errorMessage = (error instanceof ValidationError || error instanceof TimezoneError) 
          ? (error as ValidationError | TimezoneError).message 
  ...
  server.registerTool(
    ...
    async ({ zipCode, businessType = "dental" }: { zipCode: string; businessType?: "dental" | "medical" | "general" }) => {
      try {
  ...
  server.registerTool(
    ...
    async ({ zipCode, date, businessType = "dental" }: { zipCode: string; date: string; businessType?: "dental" | "medical" | "general" }) => {
      try {
