 ...
 */
function checkHoliday(date, timezone) {
    try {
        let checkDate;
        ...
        const holidayMatch = holidays.find(holiday => {
            ...
            return (holidayDate.getFullYear() === checkDate.getFullYear() &&
 ...
 */
function isBusinessDay(date, businessType) {
 ...
 */
function getUpcomingHolidays(currentTime, daysAhead = 30) {
 ...
 */
export function isDateAvailableForAppointments(date, zipCode, businessType = 'dental') {
    try {
        const { timezone } = getTimezoneFromPostalCode(zipCode);
 ...
 */
export function getNextAvailableBusinessDay(zipCode, businessType = 'dental', startDate) {
    try {
        const { timezone } = getTimezoneFromPostalCode(zipCode);
 ...
 */
export function formatBusinessHoursForVoice(businessType, timezone) {
    const config = BUSINESS_HOURS_CONFIG[businessType];
    const lines = [];
 ...
 */
export function getBusinessHoursSummary(businessType, zipCode) {
    const config = BUSINESS_HOURS_CONFIG[businessType];
 ...
 */
export function getDateTimeInfo(zipCode) {
    try {
        const { timezone } = getTimezoneFromPostalCode(zipCode);
        const currentTime = moment.tz(timezone);
        const todayHoliday = checkHoliday(currentTime);
